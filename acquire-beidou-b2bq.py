...
parser = optparse.OptionParser(usage="""acquire-beidou-b2bq.py [options] input_filename sample_rate carrier_offset

Acquire BeiDou B2bq signals

Examples:
  Acquire all BeiDou PRNs using standard input with sample rate 69.984 MHz and carrier offset 15.498375 MHz:
    acquire-beidou-b2bq.py /dev/stdin 69984000 15498375
...
prns = (list(b2bq.b2bq_strings.keys()) if not options.prn else util.parse_list_ranges(options.prn))
