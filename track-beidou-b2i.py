...
def track(x,s):
  ...
  cf = (s.code_f+s.carrier_f/590.0)/fs
...
parser = optparse.OptionParser(usage="""track-beidou-b2i.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track BeiDou B2I signal

Examples:
  Track with default options:
    track-beidou-b2i.py /dev/stdin 69984000 -20587126 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-beidou-b2i.py --carrier-phase 0.214 /dev/stdin 69984000 -20587126 31 1200.0 831.15
...
while True:
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-b1i.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
