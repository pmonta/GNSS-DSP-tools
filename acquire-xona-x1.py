...
def search(x,prn,doppler_search,ms):
  ...
  incr = float(x1p.code_length)/n
  c = x1p.code(prn,0,0,incr,n)                      # obtain samples of the C/A code
  c = fft.fft(c)
  ...
  for doppler in np.arange(doppler_min,doppler_max,doppler_incr):        # doppler bins
    ...
    if metric>m_metric:
      m_metric = metric
      m_code = x1p.code_length*(float(idx)/n)
      m_doppler = doppler
...
parser = optparse.OptionParser(usage="""acquire-xona-x1.py [options] input_filename sample_rate carrier_offset

Acquire Xona X1 signals

Examples:
  Acquire Xona using standard input with sample rate 69.984 MHz and carrier offset +8.567625 MHz:
    acquire-xona-x1.py /dev/stdin 69984000 8567625
  Acquire Xona with a custom doppler search grid and integration time of 20 ms:
    acquire-xona-x1.py --doppler-search -50000,50000,500 --time 20 /dev/stdin 69984000 8567625
  Acquire Xona from raw sample file "recording.iq":
    acquire-xona-x1.py recording.iq 69984000 8567625

Arguments:
  input_filename    input data file, i/q interleaved, 8 bit signed
  sample_rate       sampling rate in Hz
  carrier_offset    offset to X1 carrier in Hz (positive or negative)""")
...
# wipe off nominal offset from channel center to Xona X1 carrier
