...
import gnsstools.galileo.e5ai as e5ai
...
def track(x,s):
  ...
  p_early = e5ai.correlate(x, s.prn, 0, s.code_p-0.2, cf, e5ai.e5ai_code(prn))
  p_prompt = e5ai.correlate(x, s.prn, 0, s.code_p, cf, e5ai.e5ai_code(prn))
  p_late = e5ai.correlate(x, s.prn, 0, s.code_p+0.2, cf, e5ai.e5ai_code(prn))
...
# code loop
  ...
  s.code_p = np.mod(s.code_p,e5ai.code_length)
...
parser = optparse.OptionParser(usage="""track-galileo-e5ai.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track Galileo E5ai signal

Examples:
  Track with default options:
    track-galileo-e5ai.py /dev/stdin 69984000 -15191625 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-galileo-e5ai.py --carrier-phase 0.214 /dev/stdin 69984000 -15191625 31 1200.0 831.15
