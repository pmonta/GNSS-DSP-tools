...
def track(x,s):
  ...
  p_early = e5bi.correlate(x, s.prn, 0, s.code_p-0.2, cf, e5bi.e5bi_code(prn))
  p_prompt = e5bi.correlate(x, s.prn, 0, s.code_p, cf, e5bi.e5bi_code(prn))
  p_late = e5bi.correlate(x, s.prn, 0, s.code_p+0.2, cf, e5bi.e5bi_code(prn))
...
# code loop
  ...
  s.code_p = np.mod(s.code_p,e5bi.code_length)
...
n = int(fs*0.001*((e5bi.code_length-code_offset)/e5bi.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*e5bi.code_length/fs
...
while True:
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-e5bi.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
