...
n = int(round(0.004*fs))           # number of samples per block, approx 4 ms  # fixme: noise penalty without using secondary code
fp = open(filename,"rb")
...
while True:
  ...
  print block,np.real(p_prompt),np.imag(p_prompt),s.carrier_f,s.code_f-e5bi.chip_rate,s.eml
  if do_plots:
