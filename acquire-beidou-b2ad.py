...
def search(x,prn,doppler_search,ms):
  ...
  incr = float(b2ad.code_length)/n
  c = b2ad.code(prn,0,0,incr,n)                     # obtain samples of the B2ad code
  ...
  for doppler in np.arange(doppler_min,doppler_max,doppler_incr):        # doppler bins
    ...
    if q[idx]>m_metric:
      m_metric = q[idx]
      m_code = b2ad.code_length*(float(idx)/n)
      m_doppler = doppler
  m_code = m_code%b2ad.code_length
...
parser = optparse.OptionParser(usage="""acquire-beidou-b2ad.py [options] input_filename sample_rate carrier_offset

Acquire BeiDou B2a_data signals

Examples:
  Acquire all BeiDou PRNs using standard input with sample rate 69.984 MHz and carrier offset -15.191625 MHz:
    acquire-beidou-b2ad.py /dev/stdin 69984000 -15191625
