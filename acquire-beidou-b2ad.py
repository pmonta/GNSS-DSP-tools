...
def search(x,prn):
  ...
  c = b2ad.code(prn,0,0,incr,n)                     # obtain samples of the E5b-I code
...
# example:
#   ./acquire-beidou-b2ad.py /dev/stdin 69984000 0

filename = sys.argv[1]        # input data, raw file, i/q interleaved, 8 bit signed (two's complement)
fs = float(sys.argv[2])       # sampling rate, Hz
coffset = float(sys.argv[3])  # offset to B2a BeiDou carrier, Hz (positive or negative)
