...
def track(x,s):
  ...
  p_early = e1c.correlate(x, s.prn, 0, s.code_p-0.2, cf, e1c.e1c_code(prn), e1c.boc11)
  p_prompt = e1c.correlate(x, s.prn, 0, s.code_p, cf, e1c.e1c_code(prn), e1c.boc11)
  p_late = e1c.correlate(x, s.prn, 0, s.code_p+0.2, cf, e1c.e1c_code(prn), e1c.boc11)
...
# code loop
  ...
  s.code_p = np.mod(s.code_p,e1c.code_length)
...
parser = optparse.OptionParser(usage="""track-galileo-e1c.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track Galileo E1C signal

Examples:
  Track with default options:
    track-galileo-e1c.py /dev/stdin 69984000 -9334875 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-galileo-e1c.py --carrier-phase 0.214 /dev/stdin 69984000 -9334875 31 1200.0 831.15
