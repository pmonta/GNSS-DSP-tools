...
parser = optparse.OptionParser(usage="""track-galileo-e5aq.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track Galileo E5aq signal

Examples:
  Track with default options:
    track-galileo-e5aq.py /dev/stdin 69984000 -15191625 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-galileo-e5aq.py --carrier-phase 0.214 /dev/stdin 69984000 -15191625 31 1200.0 831.15
...
n = int(fs*0.001*((e5aq.code_length-code_offset)/e5aq.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*e5aq.code_length/fs
...
while True:
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-e5aq.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
