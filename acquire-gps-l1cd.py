...
parser = optparse.OptionParser(usage="""acquire-gps-l1cd.py [options] input_filename sample_rate carrier_offset

Acquire GPS L1C_data signals

Examples:
  Acquire all GPS PRNs using standard input with sample rate 69.984 MHz and carrier offset -9.334875 MHz:
    acquire-gps-l1cd.py /dev/stdin 69984000 -9334875
  Acquire all GPS and WAAS PRNs with a custom doppler search grid and integration time of 40 ms:
    acquire-gps-l1cd.py --prn 1-32,131,133,135,138 --doppler-search -6000,6000,50 --ms 40 /dev/stdin 69984000 -9334875
  Acquire all GPS and QZSS PRNs from raw sample file "recording.iq":
    acquire-gps-l1cd.py --prn 1-32,193,194,195,199 recording.iq 69984000 -9334875
...
h = scipy.signal.firwin(161,4e6/(fs/2),window='hann')
