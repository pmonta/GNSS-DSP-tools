...
def track(x,s):
  ...
  cf = (s.code_f+s.carrier_f/124.0)/fs

  p_early = b3i.correlate(x, s.prn, 0, s.code_p-0.5, cf, b3i.b3i_code(prn))
  p_prompt = b3i.correlate(x, s.prn, 0, s.code_p, cf, b3i.b3i_code(prn))
  p_late = b3i.correlate(x, s.prn, 0, s.code_p+0.5, cf, b3i.b3i_code(prn))
...
# code loop
  ...
  s.code_p = np.mod(s.code_p,b3i.code_length)
...
parser = optparse.OptionParser(usage="""track-beidou-b3i.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track BeiDou B3I signal

Examples:
  Track with default options:
    track-beidou-b3i.py /dev/stdin 69984000 -5134125 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-beidou-b3i.py --carrier-phase 0.214 /dev/stdin 69984000 -5134125 31 1200.0 831.15
...
n = int(fs*0.001*((b3i.code_length-code_offset)/b3i.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*b3i.code_length/fs

s = tracking_state(fs=fs, prn=prn,                    # initialize tracking state
  code_p=code_offset, code_f=b3i.chip_rate, code_i=0,
...
while True:
  ...
  if s.code_p<b3i.code_length/2:
    n = int(fs*0.001*(b3i.code_length-s.code_p)/b3i.code_length)
  else:
    n = int(fs*0.001*(2*b3i.code_length-s.code_p)/b3i.code_length)
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-b3i.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
