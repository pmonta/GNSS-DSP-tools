...
def track(x,s):
  ...
  p_early = l5q.correlate(x, s.prn, 0, s.code_p-0.2, cf, l5q.l5q_code(prn))
  p_prompt = l5q.correlate(x, s.prn, 0, s.code_p, cf, l5q.l5q_code(prn))
  p_late = l5q.correlate(x, s.prn, 0, s.code_p+0.2, cf, l5q.l5q_code(prn))
...
# example:
#   ./track-gps-l5q.py /dev/stdin 68873142.857 5606571.429 12 2000.0 1855.6
...
while True:
  ...
  print block,np.real(p_prompt),np.imag(p_prompt),s.carrier_f,s.code_f-l5q.chip_rate,(180/np.pi)*np.angle(p_prompt)
