...
def track(x,s):
  ...
  p_early = e6c.correlate(x, s.prn, 0, s.code_p-0.5, cf, e6c.e6c_code(prn))
  p_prompt = e6c.correlate(x, s.prn, 0, s.code_p, cf, e6c.e6c_code(prn))
  p_late = e6c.correlate(x, s.prn, 0, s.code_p+0.5, cf, e6c.e6c_code(prn))
...
# code loop
  ...
  s.code_p = np.mod(s.code_p,e6c.code_length)
...
parser = optparse.OptionParser(usage="""track-galileo-e6c.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track Galileo E6c signal

Examples:
  Track with default options:
    track-galileo-e6c.py /dev/stdin 69984000 5095875 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-galileo-e6c.py --carrier-phase 0.214 /dev/stdin 69984000 5095875 31 1200.0 831.15
...
n = int(fs*0.001*((e6c.code_length-code_offset)/e6c.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*e6c.code_length/fs

s = tracking_state(fs=fs, prn=prn,                    # initialize tracking state
  code_p=code_offset, code_f=e6c.chip_rate, code_i=0,
...
while True:
  ...
  if s.code_p<e6c.code_length/2:
    n = int(fs*0.001*(e6c.code_length-s.code_p)/e6c.code_length)
  else:
    n = int(fs*0.001*(2*e6c.code_length-s.code_p)/e6c.code_length)
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-e6c.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
