...
def track(x,s):
  ...
  cf = (s.code_f+s.carrier_f/116.375)/fs

  p_early = x5p.correlate(x, s.prn, 0, s.code_p-0.5, cf, x5p.x5p_code(prn))
  p_prompt = x5p.correlate(x, s.prn, 0, s.code_p, cf, x5p.x5p_code(prn))
  p_late = x5p.correlate(x, s.prn, 0, s.code_p+0.5, cf, x5p.x5p_code(prn))
...
#  dll_k2 = 0.2
  ...
  s.code_p = np.mod(s.code_p,x5p.code_length)
...
parser = optparse.OptionParser(usage="""track-xona-x5p.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track Xona X5 pilot signal

Examples:
  Track with default options:
    track-xona-x5p.py /dev/stdin 69984000 -1125375 0 -1600.0 5383.3
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-xona-x5p.py --carrier-phase 0.214 /dev/stdin 69984000 -1125375 0 -1600.0 5383.3
...
n = int(fs*0.001*((x5p.code_length-code_offset)/x5p.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*x5p.code_length/fs

s = tracking_state(fs=fs, prn=prn,                    # initialize tracking state
  code_p=code_offset, code_f=x5p.chip_rate, code_i=0,
...
#    s.mode = 'PLL'
  ...
  if s.code_p<x5p.code_length/2:
    n = int(fs*0.001*(x5p.code_length-s.code_p)/x5p.code_length)
  else:
    n = int(fs*0.001*(2*x5p.code_length-s.code_p)/x5p.code_length)
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-x5p.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
