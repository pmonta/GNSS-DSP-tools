...
def search(x,prn,doppler_search,ms):
  ...
  incr = float(e5ai.code_length)/n
  c = e5ai.code(prn,0,0,incr,n)                     # obtain samples of the E5b-I code
  ...
  for doppler in np.arange(doppler_min,doppler_max,doppler_incr):        # doppler bins
    ...
    if q[idx]>m_metric:
      m_metric = q[idx]
      m_code = e5ai.code_length*(float(idx)/n)
      m_doppler = doppler
  m_code = m_code%e5ai.code_length
...
parser = optparse.OptionParser(usage="""acquire-galileo-e5ai.py [options] input_filename sample_rate carrier_offset

Acquire Galileo E5ai signals

Examples:
  Acquire all Galileo PRNs using standard input with sample rate 69.984 MHz and carrier offset -15.191625 MHz:
    acquire-galileo-e5ai.py /dev/stdin 69984000 -15191625
