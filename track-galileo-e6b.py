...
import gnsstools.galileo.e6b as e6b
...
def track(x,s):
  ...
  cf = (s.code_f+s.carrier_f/250.0)/fs

  p_early = e6b.correlate(x, s.prn, 0, s.code_p-0.5, cf, e6b.e6b_code(prn))
  p_prompt = e6b.correlate(x, s.prn, 0, s.code_p, cf, e6b.e6b_code(prn))
  p_late = e6b.correlate(x, s.prn, 0, s.code_p+0.5, cf, e6b.e6b_code(prn))
...
# code loop
  ...
  s.code_p = np.mod(s.code_p,e6b.code_length)
...
parser = optparse.OptionParser(usage="""track-galileo-e6b.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track Galileo E6b signal

Examples:
  Track with default options:
    track-galileo-e6b.py /dev/stdin 69984000 5095875 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-galileo-e6b.py --carrier-phase 0.214 /dev/stdin 69984000 5095875 31 1200.0 831.15
...
n = int(fs*0.001*((e6b.code_length-code_offset)/e6b.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*e6b.code_length/fs

s = tracking_state(fs=fs, prn=prn,                    # initialize tracking state
  code_p=code_offset, code_f=e6b.chip_rate, code_i=0,
...
while True:
  ...
  if s.code_p<e6b.code_length/2:
    n = int(fs*0.001*(e6b.code_length-s.code_p)/e6b.code_length)
  else:
    n = int(fs*0.001*(2*e6b.code_length-s.code_p)/e6b.code_length)
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-e6b.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
