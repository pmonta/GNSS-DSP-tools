...
# code loop
  ...
  s.code_p = np.mod(s.code_p,l5i.code_length)
...
parser = optparse.OptionParser(usage="""track-gps-l5i.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track GPS L5I signal

Examples:
  Track with default options:
    track-gps-l5i.py /dev/stdin 69984000 -15191625 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-gps-l5i.py --carrier-phase 0.214 /dev/stdin 69984000 -15191625 31 1200.0 831.15
...
n = int(fs*0.001*((l5i.code_length-code_offset)/l5i.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*l5i.code_length/fs

s = tracking_state(fs=fs, prn=prn,                    # initialize tracking state
  code_p=code_offset, code_f=l5i.chip_rate, code_i=0,
...
while True:
  ...
  if s.code_p<l5i.code_length/2:
    n = int(fs*0.001*(l5i.code_length-s.code_p)/l5i.code_length)
  else:
    n = int(fs*0.001*(2*l5i.code_length-s.code_p)/l5i.code_length)
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-l5i.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
