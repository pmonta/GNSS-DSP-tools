...
import gnsstools.beidou.b2bi as b2bi
...
def track(x,s):
  ...
  p_early = b2bi.correlate(x, s.prn, 0, s.code_p-0.5, cf, b2bi.b2bi_code(prn))
...
# code loop
  ...
  s.code_p = np.mod(s.code_p,b2bi.code_length)
...
parser = optparse.OptionParser(usage="""track-beidou-b2bi.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track BeiDou B2bi signal

Examples:
  Track with default options:
    track-beidou-b2bi.py /dev/stdin 69984000 15498375 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-beidou-b2bi.py --carrier-phase 0.214 /dev/stdin 69984000 15498375 31 1200.0 831.15
...
n = int(fs*0.001*((b2bi.code_length-code_offset)/b2bi.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*b2bi.code_length/fs

s = tracking_state(fs=fs, prn=prn,                    # initialize tracking state
  code_p=code_offset, code_f=b2bi.chip_rate, code_i=0,
...
while True:
  ...
  if s.code_p<b2bi.code_length/2:
    n = int(fs*0.001*(b2bi.code_length-s.code_p)/b2bi.code_length)
  else:
    n = int(fs*0.001*(2*b2bi.code_length-s.code_p)/b2bi.code_length)
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-b2bi.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late
