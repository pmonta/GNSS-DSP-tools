...
def track(x,s):
  ...
  p_early = l2cm.correlate(x, s.prn, 0, s.code_p-0.5, cf, l2cm.l2cm_code(prn))
  p_prompt = l2cm.correlate(x, s.prn, 0, s.code_p, cf, l2cm.l2cm_code(prn))
  p_late = l2cm.correlate(x, s.prn, 0, s.code_p+0.5, cf, l2cm.l2cm_code(prn))
...
# code loop
  ...
  s.code_p = np.mod(s.code_p,l2cm.code_length)
...
parser = optparse.OptionParser(usage="""track-gps-l2cm.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track GPS L2CM signal

Examples:
  Track with default options:
    track-gps-l2cm.py /dev/stdin 69984000 -127126 31 1200.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-gps-l2cm.py --carrier-phase 0.214 /dev/stdin 69984000 -127126 31 1200.0 831.15

Arguments:
  input_filename    input data file, i/q interleaved, 8 bit signed
  sample_rate       sampling rate in Hz
  carrier_offset    offset to L2 carrier in Hz (positive or negative)
  PRN               PRN
...
n = int(fs*0.020*((l2cm.code_length-code_offset)/l2cm.code_length))  # align with 20 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*50.0*l2cm.code_length/fs

s = tracking_state(fs=fs, prn=prn,                    # initialize tracking state
  code_p=code_offset, code_f=l2cm.chip_rate, code_i=0,
...
while True:
  ...
  if s.code_p<l2cm.code_length/2:
    n = int(fs*0.020*(l2cm.code_length-s.code_p)/l2cm.code_length)
  else:
    n = int(fs*0.020*(2*l2cm.code_length-s.code_p)/l2cm.code_length)
