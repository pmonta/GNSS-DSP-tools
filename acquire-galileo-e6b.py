...
def search(x,prn):
  ...
  c = e6b.code(prn,0,0,incr,n)                     # obtain samples of the E1-B code
  c = fft.fft(np.concatenate((c,np.zeros(n))))
  m_metric,m_code,m_doppler = 0,0,0
  for doppler in np.arange(-9000,9000,200):        # doppler bins
    q = np.zeros(2*n)
...
# example:
#   ./acquire-galileo-e6b.py /dev/stdin 69984000 -9334875
...
xr = np.interp((1/fsr)*np.arange(85*3*5115),np.arange(len(x)),np.real(x))
xi = np.interp((1/fsr)*np.arange(85*3*5115),np.arange(len(x)),np.imag(x))
x = xr+(1j)*xi
