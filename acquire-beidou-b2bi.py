...
parser = optparse.OptionParser(usage="""acquire-beidou-b2bi.py [options] input_filename sample_rate carrier_offset

Acquire BeiDou B2bi signals

Examples:
  Acquire all BeiDou PRNs using standard input with sample rate 69.984 MHz and carrier offset 15.498375 MHz:
    acquire-beidou-b2bi.py /dev/stdin 69984000 15498375
...
parser.add_option("--prn", default="", help="PRNs to search, e.g. 1,3,7-14,31 (default %default)")
...
prns = (list(b2bi.b2bi_strings.keys()) if not options.prn else util.parse_list_ranges(options.prn))
