...
import gnsstools.xona.x1p as x1p
...
def track(x,s):
  ...
  p_early = x1p.correlate(x, s.prn, 0, s.code_p-0.05, cf, x1p.x1p_code(prn))
  p_prompt = x1p.correlate(x, s.prn, 0, s.code_p, cf, x1p.x1p_code(prn))
  p_late = x1p.correlate(x, s.prn, 0, s.code_p+0.05, cf, x1p.x1p_code(prn))
...
# code loop
  ...
  t = np.mod(s.code_p,x1p.code_length)
...
parser = optparse.OptionParser(usage="""track-xona-x1p.py [options] input_filename sample_rate carrier_offset PRN doppler code_offset

Track Xona X1P signal

Examples:
  Track with default options:
    track-xona-x1p.py /dev/stdin 69984000 8567625 0 -21000.0 831.15
  Track with pure PLL (no FLL intervals at the start) and with a specified carrier phase:
    track-xona-x1p.py --carrier-phase 0.214 /dev/stdin 69984000 8567625 0 -21000.0 831.15
...
n = int(fs*0.001*((x1p.code_length-code_offset)/x1p.code_length))  # align with 1 ms code boundary
x = io.get_samples_complex(fp,n)
code_offset += n*1000.0*x1p.code_length/fs

s = tracking_state(fs=fs, prn=prn,                    # initialize tracking state
  code_p=code_offset, code_f=x1p.chip_rate, code_i=0,
...
#    s.mode = 'PLL'
  ...
  if s.code_p<x1p.code_length/2:
    n = int(fs*0.001*(x1p.code_length-s.code_p)/x1p.code_length)
  else:
    n = int(fs*0.001*(2*x1p.code_length-s.code_p)/x1p.code_length)
  ...
  vars = block, np.real(p_prompt), np.imag(p_prompt), s.carrier_f, s.code_f-x1p.chip_rate, (180/np.pi)*np.angle(p_prompt), s.early, s.prompt, s.late, s.code_cyc, s.code_p, s.carrier_cyc, s.carrier_p, samp
