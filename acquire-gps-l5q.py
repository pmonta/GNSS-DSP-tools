...
def search(x,prn,doppler_search,ms):
  ...
  incr = float(l5q.code_length)/n
  c = l5q.code(prn,0,0,incr,n)                      # obtain samples of the L5Q code
  ...
  for doppler in np.arange(doppler_min,doppler_max,doppler_incr):        # doppler bins
    ...
    if q[idx]>m_metric:
      m_metric = q[idx]
      m_code = l5q.code_length*(float(idx)/n)
      m_doppler = doppler
  m_code = m_code%l5q.code_length
...
parser = optparse.OptionParser(usage="""acquire-gps-l5q.py [options] input_filename sample_rate carrier_offset

Acquire GPS L5Q signals

Examples:
  Acquire all GPS PRNs using standard input with sample rate 69.984 MHz and carrier offset -15.191625 MHz:
    acquire-gps-l5q.py /dev/stdin 69984000 -15191625
  Acquire all GPS and WAAS PRNs with a custom doppler search grid and integration time of 20 ms:
    acquire-gps-l5q.py --prn 1-32,131,133,135,138 --doppler-search -6000,6000,500 --time 20 /dev/stdin 69984000 -15191625
  Acquire all GPS and QZSS PRNs from raw sample file "recording.iq":
    acquire-gps-l5q.py --prn 1-32,193,194,195,199 recording.iq 69984000 -15191625
